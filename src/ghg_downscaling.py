...
class GHGEmissionsDownscaler:
    ...
    def __init__(self, config_path: str = "configs/config_ghg.json"):
        ...
        # Import and initialize the new data loader (no mock data)
        import sys
        from pathlib import Path
        sys.path.append(str(Path(__file__).parent))
        
        from preprocessing.data_loader import RealDataLoader
        self.data_loader = RealDataLoader(config_path)
    ...
    def load_emissions_data(self) -> Dict[str, pd.DataFrame]:
        """
        Load emissions data from real sources only (no mock data)
        
        Returns:
            Dictionary of emissions datasets by source
        """
        print("\n🛰️  Loading Real GHG Emissions Data")
        print("-" * 45)
        
        # Load all available real data
        self.emissions_data = self.data_loader.get_all_available_data()
        ...
        for source, data in self.emissions_data.items():
            ...
            if len(data) > 0:
                ...
                # Identify gas columns
                gas_cols = [col for col in data.columns if any(gas in col.upper() for gas in ['CO2', 'CH4', 'N2O'])]
                if gas_cols:
                    print(f"   Gases: {gas_cols[:3]}{'...' if len(gas_cols) > 3 else ''}")
